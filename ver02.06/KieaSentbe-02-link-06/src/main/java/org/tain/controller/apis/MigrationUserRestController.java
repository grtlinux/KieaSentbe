...
@RestController
@RequestMapping(value = {"/link/migrationUser"})
@Slf4j
public class MigrationUserRestController {
	...
	public ResponseEntity<?> reqStrToJson(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("LINK.MigrationUser >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("LINK.MigrationUser >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (sentbe.contains("localhost")) {
				...
				lnsJson.setHttpUrl(sentbe + "/apis/migrationUser");
				lnsJson.setHttpMethod("POST");
				lnsJson = this.lnsHttpClient.post(lnsJson);
			} else {
				lnsJson = new ObjectMapper().readValue(reqHttpEntity.getBody(), LnsJson.class);
				lnsJson.setHttpUrl(sentbe + "/hanwha/migrationUser");
				lnsJson.setHttpMethod("POST");
