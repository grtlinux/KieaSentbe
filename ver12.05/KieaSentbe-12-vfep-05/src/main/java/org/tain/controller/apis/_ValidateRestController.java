...
@Controller
@RequestMapping(value = {"/apis/validate"})
@Slf4j
public class _ValidateRestController {
	...
	public ResponseEntity<?> mapperReqJson2Str(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			lnsJson = new LnsJson();
			lnsJson.setName("Validate mapperReqJson2Str");
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/validate/req/j2s");
	...
	public ResponseEntity<?> validate(HttpEntity<String> reqHttpEntity) throws Exception {
		log.info("KANG-20200730 >>>>> {} {}", CurrentInfo.get());
		...
		if (Flag.flag) {
			lnsJson = new LnsJson();
			lnsJson.setName("Validate lns01");
			lnsJson.setHttpUrl("http://localhost:18091/v1.0/lns01/validate");
			lnsJson.setHttpMethod("POST");
	...
	public ResponseEntity<?> mapperResStr2Json(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			lnsJson = new LnsJson();
			lnsJson.setName("Validate mapperResStr2Json");
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/validate/res/s2j");
			lnsJson.setHttpMethod("POST");
	...
	public ResponseEntity<?> mapperReqCStruct(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			lnsJson = new LnsJson();
			lnsJson.setName("Validate mapperResStr2Json");
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/validate/req/cstruct");
			lnsJson.setHttpMethod("POST");
			lnsJson.setReqJsonData(reqHttpEntity.getBody());
	...
	public ResponseEntity<?> mapperResCStruct(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			lnsJson = new LnsJson();
			lnsJson.setName("Validate mapperResStr2Json");
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/validate/res/cstruct");
			lnsJson.setHttpMethod("POST");
			lnsJson.setReqJsonData(reqHttpEntity.getBody());
