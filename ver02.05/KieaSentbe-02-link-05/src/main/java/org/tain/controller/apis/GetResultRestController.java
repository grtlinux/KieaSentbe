...
public class GetResultRestController {
	...
	public ResponseEntity<?> reqStrToJson(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("LINK.GetResult >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("LINK.GetResult >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl(this.projEnvUrlProperties.getSentbe() + "/hanwha/getResult");
			lnsJson.setHttpMethod("POST");
			lnsJson = this.lnsSentbeClient.post(lnsJson);
			log.info(">>>>> RES.GetResult.lnsJson  = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
