...
@RestController
@RequestMapping(value = {"/link/getVerification"})
@Slf4j
public class GetVerificationRestController {
	...
	public ResponseEntity<?> reqStrToJson(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("LINK.GetVerification >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("LINK.GetVerification >>>>> Body = {}", reqHttpEntity.getBody());
		}
