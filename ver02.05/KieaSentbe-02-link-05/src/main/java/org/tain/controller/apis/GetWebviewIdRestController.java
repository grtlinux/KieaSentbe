...
@RestController
@RequestMapping(value = {"/link/getWebviewId"})
@Slf4j
public class GetWebviewIdRestController {
	...
	public ResponseEntity<?> reqStrToJson(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("LINK.GetWebviewId >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("LINK.GetWebviewId >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl(this.projEnvUrlProperties.getSentbe() + "/hanwha/getWebviewId");
			lnsJson.setHttpMethod("POST");
			lnsJson = this.lnsSentbeClient.post(lnsJson);
			log.info(">>>>> RES.GetWebviewId.lnsJson  = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
