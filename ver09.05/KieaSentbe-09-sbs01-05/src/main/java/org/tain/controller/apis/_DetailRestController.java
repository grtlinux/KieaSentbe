...
@Controller
@RequestMapping(value = {"/lns01/detail"})
@Slf4j
public class _DetailRestController {
	...
	public ResponseEntity<?> detail(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/Detail >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/Detail >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				String reqTypeCode = "0200200";
				String reqLen = String.format("%04d", 7 + reqStrData.length());
				reqLnsStream = new LnsStream(reqLen + reqTypeCode + reqStrData);
				log.info("Lns01/Detail >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("Lns01/Detail >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/Detail >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
