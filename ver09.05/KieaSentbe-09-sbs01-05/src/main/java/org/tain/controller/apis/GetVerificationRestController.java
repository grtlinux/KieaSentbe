...
@Controller
@RequestMapping(value = {"/sbs01/getVerification"})
@Slf4j
public class GetVerificationRestController {
	...
	public ResponseEntity<?> getVerification(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/GetVerification >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/GetVerification >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				String reqTypeCode = "0200700";
				String reqLen = String.format("%04d", 7 + reqStrData.length());
				reqLnsStream = new LnsStream(reqLen + reqTypeCode + reqStrData);
				log.info("Lns01/GetVerification >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("Lns01/GetVerification >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/GetVerification  >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
