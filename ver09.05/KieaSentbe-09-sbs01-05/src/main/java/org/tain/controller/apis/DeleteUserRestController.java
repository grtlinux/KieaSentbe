...
@Controller
@RequestMapping(value = {"/sbs01/deleteUser"})
@Slf4j
public class DeleteUserRestController {
	...
	public ResponseEntity<?> deleteUser(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/DeleteUser >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/DeleteUser >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				log.info("Lns01/DeleteUser >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("Lns01/DeleteUser >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/DeleteUser  >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
