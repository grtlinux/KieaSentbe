...
@Controller
@RequestMapping(value = {"/sbs01/createUser"})
@Slf4j
public class CreateUserRestController {
	...
	public ResponseEntity<?> createUser(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/CreateUser >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/CreateUser >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				log.info("Lns01/CreateUser >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/CreateUser  >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
