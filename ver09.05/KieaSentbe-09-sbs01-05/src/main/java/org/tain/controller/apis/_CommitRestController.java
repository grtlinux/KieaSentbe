...
@Controller
@RequestMapping(value = {"/lns01/commit"})
@Slf4j
public class _CommitRestController {
	...
	public ResponseEntity<?> commit(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/Commit >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/Commit >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				log.info("Lns01/Commit >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("Lns01/Commit >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/Commit  >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
