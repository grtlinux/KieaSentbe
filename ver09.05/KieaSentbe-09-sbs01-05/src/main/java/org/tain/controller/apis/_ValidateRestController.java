...
@Controller
@RequestMapping(value = {"/lns01/validate"})
@Slf4j
public class _ValidateRestController {
	...
	public ResponseEntity<?> validate(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/Validate >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/Validate >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				String reqTypeCode = "0200300";
				String reqLen = String.format("%04d", 7 + reqStrData.length());
				reqLnsStream = new LnsStream(reqLen + reqTypeCode + reqStrData);
				log.info("Lns01/Validate >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("Lns01/Validate >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/Validate >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
