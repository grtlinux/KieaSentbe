...
@Controller
@RequestMapping(value = {"/lns01/histories"})
@Slf4j
public class _HistoriesRestController {
	...
	public ResponseEntity<?> histories(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/Histories >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/Histories >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				log.info("Lns01/Histories >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("Lns01/Histories >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/Histories >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
