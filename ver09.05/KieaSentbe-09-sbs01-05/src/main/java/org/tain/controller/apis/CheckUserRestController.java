...
@Controller
@RequestMapping(value = {"/sbs01/checkUser"})
@Slf4j
public class CheckUserRestController {
	...
	public ResponseEntity<?> checkUser(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			log.info("Lns01/CheckUser >>>>> Headers = {}", reqHttpEntity.getHeaders());
			log.info("Lns01/CheckUser >>>>> Body = {}", reqHttpEntity.getBody());
		}
		...
		if (Flag.flag) {
			...
			if (Flag.flag) {
				...
				log.info("Lns01/CheckUser >>>>> reqLnsStream = {}", JsonPrint.getInstance().toPrettyJson(reqLnsStream));
			}
			...
			if (Flag.flag) {
				...
				log.info("Lns01/CheckUser >>>>> resLnsStream = {}", JsonPrint.getInstance().toPrettyJson(resLnsStream));
			}
			
			lnsJson.setResStrData(resLnsStream.getContent());
			log.info("Lns01/CheckUser  >>>>> lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
		}
