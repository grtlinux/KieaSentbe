...
public class GetResultProcess {
	...
	public LnsStream process(LnsStream reqLnsStream) throws Exception {
		...
		if (Flag.flag) {
			// 1. LnsJson
			lnsJson = LnsJson.builder().name("GetResult").build();
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:17087/v0.5/mapper/getResult/req/s2j");
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:17087/v0.5/mapper/getResult/res/j2s");
			lnsJson.setHttpMethod("POST");
			lnsJson = this.lnsHttpClient.post(lnsJson);
