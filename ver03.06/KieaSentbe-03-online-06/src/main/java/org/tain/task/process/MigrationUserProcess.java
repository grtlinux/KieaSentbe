...
@Component
@Slf4j
public class MigrationUserProcess {
	...
	public LnsStream process(LnsStream reqLnsStream) throws Exception {
		...
		if (Flag.flag) {
			// 1. LnsJson
			lnsJson = LnsJson.builder().name("MigrationUser").build();
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:17087/v0.5/mapper/migrationUser/req/s2j");
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:17087/v0.5/mapper/migrationUser/res/j2s");
			lnsJson.setHttpMethod("POST");
			lnsJson = this.lnsHttpClient.post(lnsJson);
