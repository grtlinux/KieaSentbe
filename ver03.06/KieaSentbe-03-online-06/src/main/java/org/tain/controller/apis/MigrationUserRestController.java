...
@Controller
@RequestMapping(value = {"/online/migrationUser"})
@Slf4j
@Deprecated
public class MigrationUserRestController {
	...
	/*
	 * url: http://localhost:18083/v1.0/online/migrationUser
	 */
	...
	public ResponseEntity<?> migrationUser(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:17082/v0.5/link/migrationUser");
			lnsJson.setHttpMethod("POST");
			lnsJson.setReqJsonData(reqHttpEntity.getBody());
			log.info("ONLINE:MigrationUser >>>>> REQ.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
			
			lnsJson = this.lnsHttpClient.post(lnsJson);
			log.info("ONLINE:MigrationUser >>>>> RES.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
