...
@Controller
@RequestMapping(value = {"/online/deleteUser"})
@Slf4j
public class DeleteUserRestController {
	...
	/*
	 * url: http://localhost:18083/v1.0/online/deleteUser
	 */
	...
	public ResponseEntity<?> deleteUser(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			log.info("ONLINE:DeleteUser >>>>> REQ.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
			
			lnsJson = this.lnsHttpClient.post(lnsJson);
			log.info("ONLINE:DeleteUser >>>>> RES.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
