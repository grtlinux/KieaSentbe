...
@Controller
@RequestMapping(value = {"/online/getWebviewId"})
@Slf4j
@Deprecated
public class GetWebviewIdRestController {
	...
	/*
	 * url: http://localhost:18083/v1.0/online/getWebviewId
	 */
	...
	public ResponseEntity<?> getWebviewId(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			log.info("ONLINE:GetWebviewId >>>>> REQ.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
			
			lnsJson = this.lnsHttpClient.post(lnsJson);
			log.info("ONLINE:GetWebviewId >>>>> RES.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
