...
@Controller
@RequestMapping(value = {"/online/getVerification"})
@Slf4j
public class GetVerificationRestController {
	...
	/*
	 * url: http://localhost:18083/v1.0/online/getVerification
	 */
	...
	public ResponseEntity<?> getVerification(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:17082/v0.5/link/getVerification");
			lnsJson.setHttpMethod("POST");
			lnsJson.setReqJsonData(reqHttpEntity.getBody());
			log.info("ONLINE:GetVerification >>>>> REQ.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
			
			lnsJson = this.lnsHttpClient.post(lnsJson);
			log.info("ONLINE:GetVerification >>>>> RES.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
