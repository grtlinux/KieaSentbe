...
@Controller
@RequestMapping(value = {"/online/refund"})
@Slf4j
public class _RefundRestController {

	/*
	 * url: http://localhost:18083/v1.0/online/refund
	 */
	...
	public ResponseEntity<?> refund(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			log.info("ONLINE:Refund >>>>> REQ.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
			
			lnsJson = LnsHttpClient.post(lnsJson);
			log.info("ONLINE:Refund >>>>> RES.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
