...
@Controller
@RequestMapping(value = {"/online/auth"})
@Slf4j
public class _AuthRestController {

	/*
	 * url: http://localhost:18083/v1.0/online/auth
	 */
	...
	public ResponseEntity<?> auth(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18082/v1.0/link/auth");
			lnsJson.setHttpMethod("POST");
			lnsJson.setReqJsonData(reqHttpEntity.getBody());
			log.info("ONLINE:Auth >>>>> REQ.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
			
			lnsJson = LnsHttpClient.post(lnsJson);
			log.info("ONLINE:Auth >>>>> RES.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
