...
@Controller
@RequestMapping(value = {"/online/detail"})
@Slf4j
public class _DetailRestController {

	/*
	 * url: http://localhost:18083/v1.0/online/detail
	 */
	...
	public ResponseEntity<?> detail(HttpEntity<String> reqHttpEntity) throws Exception {
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18082/v1.0/link/detail");
			lnsJson.setHttpMethod("POST");
			lnsJson.setReqJsonData(reqHttpEntity.getBody());
			log.info("ONLINE:Detail >>>>> REQ.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
			
			lnsJson = LnsHttpClient.post(lnsJson);
			log.info("ONLINE:Detail >>>>> RES.lnsJson = {}", JsonPrint.getInstance().toPrettyJson(lnsJson));
