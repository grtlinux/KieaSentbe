...
public class _AuthProcess {
	...
	public LnsStream process(LnsStream reqLnsStream) throws Exception {
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/auth/req/s2j");
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/auth/res/j2s");
			lnsJson.setHttpMethod("POST");
			lnsJson = LnsHttpClient.post(lnsJson);
