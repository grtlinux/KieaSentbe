...
@Component
@Slf4j
public class _HistoriesProcess {
	...
	public LnsStream process(LnsStream reqLnsStream) throws Exception {
		...
		if (Flag.flag) {
			// 1. LnsJson
			lnsJson = LnsJson.builder().name("Histories").build();
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/histories/req/s2j");
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/histories/res/j2s");
			lnsJson.setHttpMethod("POST");
			lnsJson = LnsHttpClient.post(lnsJson);
