...
public class _CommitProcess {
	...
	public LnsStream process(LnsStream reqLnsStream) throws Exception {
		...
		if (Flag.flag) {
			// 1. LnsJson
			lnsJson = LnsJson.builder().name("Commit").build();
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/commit/req/s2j");
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/commit/res/j2s");
			lnsJson.setHttpMethod("POST");
			lnsJson = LnsHttpClient.post(lnsJson);
			//lnsJson.setResStrData("Hello, world!!! Commit");
