...
public class _ValidateProcess {
	...
	public LnsStream process(LnsStream reqLnsStream) throws Exception {
		...
		if (Flag.flag) {
			// 1. LnsJson
			lnsJson = LnsJson.builder().name("Validate").build();
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/validate/req/s2j");
		...
		if (Flag.flag) {
			...
			lnsJson.setHttpUrl("http://localhost:18086/v1.0/mapper/validate/res/j2s");
			lnsJson.setHttpMethod("POST");
			lnsJson = LnsHttpClient.post(lnsJson);
			//lnsJson.setResStrData("Hello, world!!! Validate");
