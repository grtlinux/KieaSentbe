...
public class _LnsMstInfo implements Cloneable {
	...
	public _LnsMstInfo(String strJsonMstInfo) throws Exception {
		log.info("KANG-20200721 >>>>> {} {}", CurrentInfo.get());
		
		JsonNode jsonNode = null;
		if (Flag.flag) {
			// check
			jsonNode = JsonPrint.getInstance().getObjectMapper().readTree(strJsonMstInfo);
			
			JsonNode node = null;
			node = jsonNode.at("/__headBase");
			if (node == null || node.isNull() || node.isEmpty()) {
				throw new RuntimeException("wrong json MstInfo format.. Thers's no __headBase");
			}
			node = jsonNode.at("/__headData");
			if (node == null || node.isNull() || node.isEmpty()) {
				throw new RuntimeException("wrong json MstInfo format.. Thers's no __headData");
			}
			node = jsonNode.at("/__bodyBase");
			if (node == null || node.isNull() || node.isEmpty()) {
				throw new RuntimeException("wrong json MstInfo format.. Thers's no __bodyBase");
			}
			node = jsonNode.at("/__bodyData");
			if (node == null || node.isNull() || node.isEmpty()) {
				throw new RuntimeException("wrong json MstInfo format.. Thers's no __bodyData");
			}
		}
	...
	public _LnsMstInfo() throws Exception {
		log.info("KANG-20200721 >>>>> {} {}", CurrentInfo.get());
